...
- **Resume 长度**：**2.5–3 页 A4**
- **字数预算**：1200–1800 词（中文约 2000–3000 字）

**为什么不是 2 页？**

候选人不是 5 年经验的高级开发，而是 15 年研发经验、Team Lead、多业务领域（网关、安全、风控、IM、数据安全）、三门主要语言（Go / Rust / C++），涉及架构设计、SDL、安全平台。如果硬压成 2 页，会导致每个项目只能写几行、项目亮点讲不清楚、技术深度体现不出来、ATS 关键词被迫删减。国内互联网招聘对 P7 / 技术专家 / 架构师 / 安全专家通常接受 3 页左右。真正决定面试率的不是页数，而是第一页是否吸引人以及每页阅读成本是否可控。

### 18.2 三页布局建议

| 页码 | 内容 | 目标 |
|------|------|------|
| **Page 1** | Summary + Core Skills + 最近两家公司 | 30 秒内让招聘经理知道「你是谁」 |
| **Page 2** | 闪捷最重要的 2–3 个项目 + 玩物得志核心项目 | 证明最近 5 年的深度与影响力 |
| **Page 3** | 蘑菇街及更早经历 + 教育 + CISSP + 开源/专利 | 证明成长轨迹与长期工程纪律 |

**关键原则**：第一页信息密度最高但要有留白；第二页开始讲项目；第三页以时间线为主。

### 18.3 篇幅分配

| 模块 | 占比 | 说明 |
|------|------|------|
| 最近 5-10 年经历 | ~65% | 闪捷 + 玩物得志 |
| 中期经历 | ~20% | 蘑菇街 |
| 早期经历 | ~10% | 千方/中交兴路 + 华为 |
| 教育 / 证书 / 其他 | ~5% | 学历、CISSP、专利、开源 |

### 18.4 每家公司 Bullet 数量建议
...
- 每页不要出现大段文字块。
- 每个 bullet 控制在 1–2 行。
- 关键量化结果（QPS、RT、成本）使用加粗或独立成段。
- 第一页必须在 30 秒内建立三个印象：
  1. 15 年系统研发经验，技术基础扎实。
  2. 既能做高性能后端，也能做企业安全平台与 SDL。
  3. 具备 Team Lead 和跨项目落地能力，不只是单点编码。
...
| v3.1 | 2026-07-20 | 根据反馈调整 Resume Budget：从 2 页改为 2.5–3 页，增加三页布局建议与阅读成本控制 |
